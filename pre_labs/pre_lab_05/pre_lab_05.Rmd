...
**Answer** They have the same column names, but for some years UNA and OTH are reversed. That doesn't matter, though, because the same header columns are present in all csvs.
...
**Answer**
There should be 24 because we are doing a left join, so the # of rows will be based on the # of rows in maryland_population (24) even though county_voters_2020 has 25 rows.
```{r}
...
**Answer** Joining with `by = join_by(COUNTY)` This message tells you what field the join function is combining.
...
** the order that (COUNTY==NAME is in matters -- county belongs to the first data frame, name goes to the second, R will always try to combine a left combine working left to right )
...
**Answer** Prince George's county has the highes 18+ Black population (60.9%) and is 78.3% democrat.
...
**Answer**
Howard County has the highest percentage of Asians (19.3%) and is 52.4% democrat.
```{r}
maryland_population_with_voters_and_demographics |>
...
** when you change the data type, just save onto your current dataframe (because you aren't making anything new, just improving existing frame)

**Answer** ZCTA5N is now numeric data type.
