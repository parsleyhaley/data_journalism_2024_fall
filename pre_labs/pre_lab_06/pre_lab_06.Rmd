...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** R is reading the first row of data as headers because the .csv currently does not have headers. Some of the text from the headers is being incorporated into the row names - "Josh Saul - R\rWeldon ED", for example.	
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** The column names are still not descriptive. Some of the row names still have some characters from the old headings-- for example "Lisa Trombley - RLake ED"	
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** The data is much more decipherable. The candidate and party, combined above, are separated into two rows, along with the office they're running for. We also have headers that are more descriptive now. However, there are still issues with the data, especially on the second page. There's a NA row that wasn't eliminated earlier, and some of the text from the old headers still appears attached to entries in the candidate and party columns. 
