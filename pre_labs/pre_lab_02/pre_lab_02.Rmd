...
**Answer** Select is used to identify the columns that will appear in the table. Mutate creates a new column where the value is the value of the election day column divided by the value of the votes column.
...
**Answer** The results are now arranged by election day percentage in descending order because the arrange(desc)) function was used. Before, the results were arranged alphabetically by county name (but I'm not sure why, I didn't see an arrange function with this instruction used above).
...
**Answer** This is interesting because it allows us to see that some candidates did not receive any votes on election day.(I'm not sure otherwise why this is more meaningful than the data above?)
...
**Answer** str_to_lower() converts string to lowercase, and str_to_title() converts to title case (where only the first letter of each word is capitalized)
...
**Answer** The mutate statement is creating a new column called amount_category, then analyzing the value of the "amount" column and labeling it based on the value. The label is placed in the amount_category column.
...
**Answer** To analyze the data set, it's helpful to save it as a new variable so that we can perform various functions on it. 
...
**Answer:** The date in the expenditure_date column is now formatted year_month_day and is a day datat
...
**Answer** The data is organized by year-month-day. The day is set to one for every date to standardize the dates so that the grouping by month can take place within the day datatype formating. 
