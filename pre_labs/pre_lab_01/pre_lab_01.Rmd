...
```
 NOTE I'm getting the error message above, I think it's because I need more space on my drive to install all the packages. I'm getting a new cpu so this problem should be resolved soon, but this hasn't impacted my ability to do classwork so far.
 
 The name of the library should be in quotes (ie 'tidyverse')
Check in console if library installed: go to console and run library(name of library) to see if you get error code or not. 
...
**Answer** The code is sorting the contributions by date and then adding up the total number of contributions per date, then forming two columns one showing the date, organized chronologically, and one showing the total number of contributions per date. The head() function shows the first six lines of the data set.
...
**Answer** The arrange(desc(count_contribs)) code sorted the results above by number of contributions in descending order. So, the data is organized so that we can see the days in which the most contributions were made. 
...
**Answer** The data is organized by memo text (indicating the organization that the donation is for, which then indicates the person the donation is intended to support), amount of money donated, and number of contributions in descending order. So, we can see how often a particular amount was donated to a particular organization and, because the data is organized in descending order, we can clearly see the most frequently donated amount and the organization that that amount funded.
...
**Answer** This codeblock adds total_amount = sum(amount), which allows us to see the total amount raised by each organization and the total number of contributions made to each organization. The data is sorted by total amount in descending order. 
...
**Answer** This data block adds mean_amount = mean(amount), and median_amount = median(amount) to calculate the mean and median individual contribution amount by recipient, and then organized by number of individual contributions per recipient in descending order. Of the top 10 recipients, only one is for a Maryland candidate (Hogan for Maryland Inc.)
### Task 11
...
**Answer** The data is sorted by city of contribution, number of contributions from that city, the total amount of money collectively donated by that city, the mean and median amounts of individual donations per city, and the amounts of the minimum and maximum individual donations per city. The data is organized by total amount donated in descending order.
