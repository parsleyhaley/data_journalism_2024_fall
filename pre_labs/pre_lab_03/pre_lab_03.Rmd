...
**Answer** Warning: One or more parsing issues, call `problems()` on your data frame for details
...
**Answer** 1,640 rows means there were 1,640 problems. The problem that R is describing is that for the 12th and 13th columns for the problematic answers the datatype is coded for R to expect characters, but R is finding numerals instead.
...
**Answer** I'm not sure, but I don't think it shows any parsing errors.
...
**Answer** The "district" field is numeric datatype. "Precinct" is character, and "mail" is numeric.
...
**Answer**: There are 402,345 results where mail is NA. This implies that in those precincts/races, either no one voted by mail or information about votes at the precint level is not available.
...
**Answer**
There are 12 registrations for January 1900. Many of the results in the first 20 are implausibly early, suggesting that there might be errors in the data. It's possible that there are still voters alive who registed in 1949 or later, but less likely that voters who registered in 1933 are alive, and impossible that voters who registered in 1900 are alive.
...
**Answer** As we discussed in class, it would be interesting to find out why 5 voters requested to be removed from the voter roll. It might also be interesting to learn more about voters who recently completed their felony convictions and can now vote, and to learn how that change in status impacts their voting behavior. 
...
**Answer** 14 rows are in this demonstration data. 
...
**Answer** The amount field's datatype is characters. 
...
**Answer** 
-the name of the first column, 1_linenumber, starts with a number
-one of the column's name is in all caps (LAST_NAME)
-There's a space in one of the column's names (address one)
```{r}
conowingo
```
...
**Answer** Cleannames adds a number in front of the same 1_linenumber to prevent the column name from starting with a number.
...
**Answer** The error says that the "amount" column is the wrong charactertype (not numerals), so R is unable to sum the values.
...
**Answer** the new data type is dbl.
...
**Answer** There are two duplicated rows (two rows that are the same). The donor name is Derrick Hamilton.
...
**Answer** The last four digits were removed from any zip code longer than 5 numerals.
...
**Answer** Two mispellings of Conowingo remain after running this code. 
...
**Answer** Only one mispelling remains.
...
**Answer** No mispellings remain after running this code. 
