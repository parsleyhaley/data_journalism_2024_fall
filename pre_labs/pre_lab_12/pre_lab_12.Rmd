...
**Task** Run the following code and describe the dataframe it outputs. **Answer** A dataframe with all of Cardin's press releases, including date, title, URL, and the full text of the release.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The function identifies each unique word (filtering out repetitive words) but it recognizes differences in capitalization as unique words, even if the word itself is the same. It also recognizes punctuation as a word.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It creates a table of all the words used in the order that they appear, but makes every word lowercase and filters out punctuation.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The graphic shows that articles and conjunctions like "the" and "and" are most commonly used. This is not surprising given how the english language works, and it's not particularly interesting. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** Yes, after we remove the stop words we get a more substantive list of terms that we can use to help understand Cardin's prioritizes (as represented by the number of times a word appears)
...
**Task** Run the following code and describe the results. **Answer** The code produces two different tibbles, one for the top unique words for each year. We can see changes and trends that reflect current events -- for example, the popularity of the term "health" in 2022, when the conuntry was just starting to come out of the COVID pandemic.
...
**Task** Run the following code and describe the results. **Answer** The results are a table of the most frequently used bigrams, like "amount included" and "health care". There's also a column for the # of times the bigram appears and for the percentage of the total words that the bigram represents. This is interesting data that allows us to see Cardin's priorities, but there are a few wonky entries-- for example, "prince george's" and "george's county" become bigrams, but they're words that are usually used together. 
...
**Task** Run the following code and describe the results. **Answer** The terms are really similar -- there's some slight adjustment of position on the chart but in general, many of the same words appear. Covid 19 is no longer included, which may be indicative of the fact that covid is less newsworthy now that the pandemic is over.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer** Stem_words groups words only if they have the same stem (drive and drives, for example) which lemmatize_words groups words that are different tenses, even if they have different stems (like drive and drove)
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** The lemmitized version is more useful, because it groups words that, especially for our analysis, are so similar that we can treat them as the same-- like "funds" and "funding". It also does this with "community" and "communities". By combining those terms, we get a better sense of the frequently used words and have access to more information because the top ten list includes more unique words.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** Yes, for example "issues" is negative-- that word can be used to mean "problems", which has a negative sentiment usually. But it could also mean "topics", which doesn't have a negative sentiment. 
