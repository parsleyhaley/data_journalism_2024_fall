...
**Answer**
This code adds a column using mutate to show the difference between the number of unaffiliated voters in 2024 vs 2022. It then creates a new dataframe and arranges the results showing the differences arranged from highest number to lowest and displays the top 10 rows. 
...
**Answer** A blank rectangle. There's nothing in the rectangle because we haven't written anything to tell ggplot what to make.
...
**Answer** The information from the maryland_top_counties dataframe is in a bar chart. The x-axis is the county names and the y-axis (weight) is the difference in unaffiliated voters.
...
**Answer** The data has been reordered in ascending order by different in unaffiliated voters (the y axis)
...
**Answer**The chart has been flipped to a horizontal bar chart, so the columns that were vertical are now horizontal. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The color of the graph has been changed from two different grey tones to grey and white.
...
**Answer** The background of the chart is light blue, and the horizontal lines running across the graph are white. 
...
**Answer**
This chart now has a header. The x and y axis also have titles, although they should be reversed. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The number of contributions tracked by winred by day. 
...
**Answer** The line chart shows the number of contributions tracked by winred that were made each day from April 2022 to May 2022. The chart shows a large increase in contributions at the end of July.
...
**Answer** The chart is labelled by week so that it's easier to see the dates that donations were made There's a line that is labelled with the date of the start of each week. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The featured date labels have been roatated 45 degrees so that they fit more easily below the chart.
