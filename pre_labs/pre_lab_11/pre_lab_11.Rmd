...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The columns have information beyond just the candidate name-- most have the candidate and vice presidential candidates, and the names aren't well formatted for R because there are periods and commas. 
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to?  **Answer** the numbers refer to the column number. 
...
results <- results[[1]] |>
 ...
 mutate(sharon_haris = as.numeric(gsub(",","", sharon_harris))) 
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** The output auggests that there is at least one ul tags, because multiple li tags are contained within a ul tag.

```{r}
cmte_results |> html_elements('li')
```

html_elements('li') grabs all the li tags
html_element grabs only the first li tag
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** Rows separated by new line characters and empty space in quotiations. The rows that start with new line characters are the names of pages with information about the house (the names of the tabs from the navigation bar on the site.) Rows without the newline character are the actual names of the committees listed on the site.
```{r}
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** The committees datagrame is a list of 55, meaning r has broken it down into 55 separate tabs.
...
NOTE the ^ means start with
and the ! means get rid of