...
```{r echo=FALSE}
census_api_key("81a40875bb5e8126360f71dfe6a272f3cc7604fe", install=TRUE)
```
...
The bars between Calvery and Montgomery overlap, meaning that the difference between the two is within the margin of error and the differences aren't statistically significant. 

Is the difference between Howard and everyone else significant? **Answer**
Yes, Howard does not overlap with any other county's error bars (it's pretty far away from the next highest income county, Calvert)
...
**Answer** 
No counties' median income fell. 
```{r}
md10 |>
...
```{r}
md_zips_with_acs |> arrange(desc(total))
...
**Answer**
What counties are these zip codes located in? How many people live in each zip code? What factors impact the amount given? 

```{r}
md_zips_with_acs |> arrange(desc(estimate))
```
