...
**A1.**
https://datawrapper.dwcdn.net/9LIRS/1/
This chart shows that in 2020, there was a huge jump in Marylanders voting by mail compared to 2018. During that election year, many voters voted by mail because of the pandemic-- Maryland made this more accessible by automatically sending every voter an application to vote by mail (https://www.washingtonpost.com/graphics/2020/politics/vote-by-mail-states/).The story here is that after the election, many Marylanders chose to vote in-person, as they had before the election. But, voting by mail is now more popular than before the election -- perhaps because some Marylanders enjoyed mail-in voting, or found it more convenient, after being exposed to it during the pandemic. That's probably the thesis I would follow up on with my reporting. 
...
```{r}
md_women <- read_csv("data/ev_totals_women.csv")
```
...
**A2.**  I picked this CSV because I was curious about the days of the week that women vote early -- I wondered if more women voted on weekends or during the week-- when would they be able to make time to vote? Once I saw that women voted more frequently during the work week, I wanted to see if this held true for men as well, or if they had different scheduling patterns than women. Once I made the chart, I saw that both genders tend to head to the polls on the same days of the week, but that more women voted early than men. 

https://datawrapper.dwcdn.net/nVYw2/1/ 

```{r}
md_women <- read_csv("data/ev_totals_women.csv")
md_men <- read_csv("data/ev_totals_men.csv")

md_total_day_W <- md_women |> 
...
colnames(md_total_day_W) <- c("Day", "W_total")

md_total_day_M <- md_men |> 
  mutate(Total = Allegany + `Anne Arundel` + `Baltimore City` + `Baltimore County` + Calvert + Caroline + Carroll + Cecil + Charles + Dorchester + Frederick + Garrett + Harford + Howard + Kent + Montgomery + `Prince George's` + `Queen Anne's` + `Saint Mary's` + Somerset + Talbot + Washington + Wicomico + Worcester) |> 
  select(Day, Total)

colnames(md_total_day_M) <- c("Day","M_total")

md_total_both_MW <- md_total_day_W  |> 
  left_join(md_total_day_M)


write_csv(md_total_both_MW, "data/md_total_both_MW.csv")
...
**A3.** 
MAP 1(ActRed) : https://datawrapper.dwcdn.net/X5XQI/2/
MAP 2(ActBlue) : https://datawrapper.dwcdn.net/T2c17/2/

I would be interested to know the demographics of each area that has especially high numbers of donations per capita, particularly the number of registered democrats and republicans. Since they appear to be disparate areas around the state, it would be interesting to see why these zip codes stand out on the map.
...
actblue_totals <- read_csv("data/md_actblue_zip_totals.csv")
...
```
```{r}
md_pop <- get_acs(geography="zcta", variables = "B01003_001", state='MD', year=2019) 
...
```{r}
winred_pop <- md_winred_zip_totals |> left_join(md_pop, (join_by(zip == `GEOID`)))
winred_pop <- md_winred_zip_totals |> right_join(md_pop, (join_by(zip == `GEOID`)))


winred_pop <- winred_pop |> 
    mutate(rate = (donations/estimate)*1000) 

write.csv(winred_pop,"~/Downloads/winred_pop.csv")
...
actblue_totals <- actblue_totals |> 
  mutate(zip = as.character(zip))


actblue_pop <- actblue_totals  |> left_join(md_pop, (join_by(zip == `GEOID`)))
actblue_pop <- actblue_totals  |> right_join(md_pop, (join_by(zip == `GEOID`)))



actblue_pop <- actblue_pop |> 
    mutate(rate = (donations/estimate)*1000) 

write.csv(actblue_pop,"~/Downloads/actblue_pop.csv")
